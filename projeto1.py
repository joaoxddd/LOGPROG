...
def verificaHorario (horario, numeroDeHorarios):
...
def imprimeHorario(formula, horario, choques, numeroDeChoques):
    ...
    for i in range(len(formula)):
        j = j + 1
        if formula[i] == '1':
            print('Curso{0} alocado no horario H{1}'.format(vetorIndice[j],horario))
            vetorIndiceRetirados.append(vetorIndice[j])
            del(vetorIndice[j])
            j = j - 1
    if (len(vetorIndice)) == 1:
        horario = horario + 1
        print('Curso{0} alocado no horario H{1}'.format(vetorIndice[0],horario))
    else:
        horario = horario + 1
        choques2 = retiraChoques(choques, vetorIndiceRetirados,numeroDeChoques)
    ...
    return verificaHorario(horario,numeroDeHorarios)
		...
		for k in range(a): #0-3
			for j in range(len(choquesA)):
				...
				if p == vetorIndiceA[k]:
					...
					while l<a:
						if  q == vetorIndiceA[l]:
							resultado = rAnd(int(matrizTabelaVerdade[i][k]),int(matrizTabelaVerdade[i][l]))
...
#numeroDeCursos = int(input('Entre com o número de cursos a serem organizados: '))
#cursos = [str(input("Nome do Curso: ")) for i in range(numeroDeCursos)]
numeroDeHorarios = int(input('Entre com o número de Horarios a serem organizados: '))
numeroDeChoques = int(input('Entre com o número de choques a serem organizados: '))
choques = [str(input("Entre com os choques de cursos por alunos com base na ordem apresentada na forma Exemplo(1 2): ")) for i in range(numeroDeChoques)]
numeroDeCursos = 5
